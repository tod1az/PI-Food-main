  ...
  switch (type) {
    ...
    case FILTER_DIETS:
      ...
      if (payload === 'All') {
        ...
        recipesFiltered = allrecipes.filter((recipe) => {
    ...
    case FILTER_BY_SOURCE:
      ...
      if (state.currentDiets === 'All') {
        ...
        if (payload === 'DB') {
          filteredBySource = Copystate.filter(
            (recipe) => typeof recipe.id !== 'number' && recipe.diets.find((diet) => diet.name === state.currentDiets)
          )
        }
        if (payload === 'API') {
